...
#define __LW_CNN__
	...
	float* lw_filter_conv_computing(lw_head_t* head, float* input_images, float* filter_weight, lw_layer_t layer);//一个filter卷积计算 - one filter convolution calculation
	float* lw_channel_conv_computing(lw_head_t* head, float* channel_image, float* channel_weight, lw_layer_t layer);//一个channel卷积计算 - one channel convolution calculation
	float* lw_channel_pool_computing(lw_head_t* head, float* channel_image, lw_layer_t layer);//池化 - pooling
